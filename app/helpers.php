...
if ( ! function_exists( 'htsa_get_license_api_response' ) ) {
    ...
    function htsa_get_license_api_response( mixed $response ) : array
    {
        ...
        switch ( $response ) {
            ...
            case 'inactive support plan':
                $message['type'] = 'error';
                $message['message'] = sprintf( __( '<b>%s</b>: You don\'t have an active support plan! Purchase a support plan now.', 'htsa' ), HTSA_THEME_NAME );
                break;

            case 'unknown license key':
            case 'invalid license key':
                $message['type'] = 'error';
                $message['message'] = sprintf( __( '<b>%s</b>: License key does not exists or is not valid! Contact support', 'htsa' ), HTSA_THEME_NAME );
                break;

            case 'unknown product':
                $message['type'] = 'error';
                $message['message'] = sprintf( __( '<b>%s</b>: License key is not associated with this product!', 'htsa' ), HTSA_THEME_NAME );
                break;
